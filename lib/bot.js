...
module.exports = (function() {
	...
	bot.prototype = {
		...
		getArticles: function(titlesOrPageIds, callback) {
			var params = {
				action: 'query',
				prop: 'revisions',
				rvprop: 'content'
			...
			// both page ID or title can be provided
			if (/^[\d\|]+$/.test(titlesOrPageIds)) {
				params.pageids = titlesOrPageIds;
				this.log("Getting content of articles #" + params.pageids + "...");
			}
			else {
				params.titles = titlesOrPageIds;
				this.log("Getting contents of " + params.titles + "...");
			}

			this.api.call(params).then(function(res) {
				callback && callback(res.info.pages);
			});
		},
